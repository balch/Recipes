...
fun rememberYouTubePlayer(
    ...
    // Centralized cleanup: automatically release the player when leaving composition
    DisposableEffect(lifecycleOwner, state) {
        lifecycleOwner.lifecycle.addObserver(state.playerView)
        onDispose {
            lifecycleOwner.lifecycle.removeObserver(state.playerView)
            state.release()
        }
    }
