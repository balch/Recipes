 ...
 */
@LLMDescription("""
    Represents a complete meal recipe with full details including 
    ingredients, measurements, and cooking instructions. 
    Note to LLM, do not create a version of this directly. 
    When asked to create and show a new meal, call the 
    `meal_recipe_create` tool to create the meal, and then the
    `navigation_meal_recipe_detail` tool to navigate to
     the detail screen.
    """)
@Serializable
data class Meal(
    @property:LLMDescription("Unique identifier for the meal. Not necessarily for recipes generated by the llm (defaults to current time")
    @SerialName("idMeal")
